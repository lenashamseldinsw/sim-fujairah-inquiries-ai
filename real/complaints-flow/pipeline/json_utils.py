...
def extract_methodology_context(methodology: Optional[Dict[str, Any]], section_keys: list[str]) -> Dict[str, Any]:
    """
    Extract specific sections from the complaints management methodology JSON.

    Args:
        methodology: The full methodology dict loaded from منهجية JSON
        section_keys: Dot-notation paths e.g. ["5_procedures.5_1_receiving_channels", "6_performance_indicators"]

    Returns:
        Dict keyed by the last path component, with the extracted section content.
        Returns empty dict if methodology is None or not found.
    """
    if not methodology:
        return {}

    sections = methodology.get("document", {}).get("sections", {})
    result = {}
