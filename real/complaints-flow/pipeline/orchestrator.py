...
# Complaints Excel header row (0-indexed; row 5 in Excel: 2 blank rows + title + blank + header)
COMPLAINTS_EXCEL_HEADER_ROW = 4
...
class PipelineOrchestrator:
    ...
    def initialize_state(self, session_id: str) -> None:
        ...
        from previous runs (which caused incorrect closure rate calculations on
        Streamlit Community Cloud). State files are still saved for within-run
        crash recovery but are never loaded on subsequent runs.
        """
        self.state_file = self.temp_dir / f"{session_id}_complaints_state.json"

        # Always start fresh — never load cached state from previous runs
        # Stale state on Streamlit Community Cloud caused incorrect numbers (80% vs 92%)
    ...
    def find_guidebook_json(self) -> bool:
        ...
        try:
            ...
            print(f"[Guidebook] JSON not found in complaints-flow/complaints-supporting-files/")
            return False
    ...
    def run_stage1_validator(self, file_path: str) -> Tuple[bool, str, Dict[str, Any]]:
        """
    ...
    def run_stage3_llm_classifier(self, progress_callback=None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Compute reclassified_count: for complaints, cases where Stage 3 LLM changed sub_classification from Stage 2
            # Map case_number to original Stage 2 sub_classification from llm_queue
            stage2_subcats = {case.get('case_number'): case.get('sub_classification') for case in self.state.llm_queue}
            reclassified_count = 0
            for llm_case in self.state.llm_classified:
                stage2_subcat = stage2_subcats.get(llm_case.case_number)
                if stage2_subcat and stage2_subcat != llm_case.sub_classification:
                    reclassified_count += 1
            self.state.reclassified_count = reclassified_count
            self.state.reclassification_rate = round(
    ...
    def run_full_pipeline(
        ...
        # Load methodology if provided
        if methodology_path:
            try:
                with open(methodology_path, 'r', encoding='utf-8') as f:
                    self.state.complaints_methodology = json.load(f)
                print(f"[Orchestrator] Loaded complaints methodology from {methodology_path}")
            except Exception as e:
                print(f"[Orchestrator] WARNING: Could not load methodology from {methodology_path}: {e}")
                results['errors'].append(f"Methodology load failed: {e}")
                # Continue anyway—methodology is optional
        ...
        # Stage 2
        if progress_callback:
            progress_callback(0.25, "تصنيف الحالات حسب القواعد", "Stage 2: Rule Classification")
