"""
generate_improvement_roadmap_section — stage6_artifacts.py companion (COMPLAINTS FLOW)
...
Second table — حل الشكاوى بشكل استباقي:
  Columns: الشكوى المتوقعة | الأثر | الحل الاستباقي المقترح | مستوى الثقة
  Proactive mitigation strategies to reduce complaint volume
...
prose (التوصية, الأثر المتوقع) using these locked inputs:
  ...
  Stage 4 → journey_map
    ...
    • case_count                → used in الأثر المتوقع ("إلغاء N+ شكوى")
...
──────────────────
{
  ...
  "proactive_complaints_table": [
    {
      "الشكوى المتوقعة": "...",
      "الأثر": "...",
      "الحل الاستباقي المقترح": "...",
...
# ──────────────────────────────────────────────────────────────────────────────
# Constants — horizon bucketing (COMPLAINTS-SPECIFIC)
# ──────────────────────────────────────────────────────────────────────────────
...
def _build_roadmap_rows(state: PipelineState) -> List[Dict[str, Any]]:
    ...
    for notif in (state.notification_opportunities or []):
        ...
        rows.append({
            ...
            "seed_impact_ar":           f"إلغاء {cases_eliminated}+ شكاوى",
    ...
    for friction in journey_sorted:
        ...
        rows.append({
            ...
            "seed_impact_ar":         f"تقليص {friction.case_count}+ شكوى — {friction_seed[:60]}",
    ...
    # Merge AI and non-AI rows, sort by horizon, then cap at 6 total (spec max)
    combined = other_rows + ai_rows_built
    combined.sort(key=lambda r: (_horizon_rank(r["horizon"]), -r["case_count"]))
    final_rows = combined[:6]
    ...
    print(
        ...
        f"  Final rows: {len(final_rows)} (capped at 6 per spec)\n"
        f"  Horizon breakdown:"
    )
...
def _build_display_proactive_complaints_rows(raw_data: Dict[str, Any]) -> List[Dict[str, str]]:
    """
    Convert LLM raw_data['proactive_complaints_table'] into display-ready dicts.

    Ensures column order matches: الشكوى المتوقعة | الأثر | الحل الاستباقي المقترح | مستوى الثقة
    """
    display = []
    for row in raw_data.get("proactive_complaints_table", []):
        display.append({
            "الشكوى المتوقعة": row.get("الشكوى المتوقعة", ""),
            "الأثر": row.get("الأثر", ""),
            "الحل الاستباقي المقترح": row.get("الحل الاستباقي المقترح", ""),
            "مستوى الثقة": row.get("مستوى الثقة", ""),
        })
    return display
...
def generate_improvement_roadmap_section(
    ...
    """
    Generate Section 8 — Improvement Roadmap (خارطة الطريق التحسينية) for COMPLAINTS flow.
    ...
    then asks the LLM to write:
      ...
      • proactive_complaints_table — common complaint types with proactive mitigation strategies
    ...
    # Extract top complaint themes from journey_map for proactive table seed
    ...
    complaint_themes = [
        {
            "cluster_ar": c.cluster_ar or c.cluster,
            "case_count": c.case_count,
            "friction_ar": c.friction_point_ar or c.friction_point,
        }
        for c in top_complaints if c.case_count > 0
    ]

    # Extract performance indicators from methodology if present
    methodology_kpis = ""
    if state.complaints_methodology:
        ...
        indicators = methodology_context.get("6_performance_indicators", {}).get("indicators", [])
        if indicators:
            methodology_kpis = "\n### أهداف الأداء من المنهجية الرسمية (الإصدار 4.0):\n"
            for ind in indicators:
                methodology_kpis += f"- {ind.get('indicator', '')}: الهدف {ind.get('target', '')}\n"
            methodology_kpis += "\nعند تقديم التوصيات، اذكر الهدف المعني من المنهجية عند الإمكان.\n"

    # ── Prompt ────────────────────────────────────────────────────────────────
    prompt = (
        'You are writing Section 8 of a formal Arabic government report on customer complaint\n'
        ...
        f'top_complaint_themes: {json.dumps(complaint_themes, ensure_ascii=False)}\n'
        f'{methodology_kpis}'
        '\n'
        '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n'
        ...
        f'     prioritised improvement plan grounded in {date_range} complaint data.\n'
        ...
        '   - Open with: "تجمع هذه الخارطة توصيات التحليلات الأربعة في منظومة أولويات متدرجة لتحسين آليات معالجة الشكاوى"\n'
        ...
        '   - Must answer: WHAT to improve in complaint handling, WHERE (in-app / SMS / portal),\n'
        ...
        '   - If case_count > 0: start with "إلغاء/تقليص/تحويل [N]+ شكوى"\n'
        ...
        '\n'
        'D. proactive_complaints_table — EXACTLY 5 rows. No more, no fewer.\n'
        '   Use top_complaint_themes as the source. If fewer than 5 themes exist, split\n'
        '   the highest-count theme into two distinct anticipated complaint variants.\n'
        '   - Use top_complaint_themes as starting point for complaint types.\n'
        '   - Columns: الشكوى المتوقعة | الأثر | الحل الاستباقي المقترح | مستوى الثقة\n'
        '   - "الشكوى المتوقعة": anticipated complaint type (e.g., "تأخر الرد على الشكاوى")\n'
        '   - "الأثر": estimated number of complaints this could prevent (e.g., "تقليص 15+ شكوى")\n'
        '   - "الحل الاستباقي المقترح": specific proactive mitigation strategy to prevent this complaint\n'
        '   - "مستوى الثقة": confidence level (عالي / متوسط / منخفض) in effectiveness\n'
        ...
        '  "proactive_complaints_table": [\n'
        '    {\n'
        '      "الشكوى المتوقعة": "<anticipated complaint type>",\n'
        '      "الأثر": "<estimated impact: تقليص N+ شكوى>",\n'
        '      "الحل الاستباقي المقترح": "<specific proactive mitigation strategy>",\n'
        '      "مستوى الثقة": "<عالي / متوسط / منخفض>"\n'
        '    }\n'
        '  ]\n'
        '}\n'
    ...
    if "proactive_complaints_table" not in raw_data or not raw_data["proactive_complaints_table"]:
        raise RuntimeError(
            "[ImprovementRoadmap] LLM response missing or empty 'proactive_complaints_table'."
        )

    # Spec requires exactly 5 rows — enforce hard
    pc_count = len(raw_data["proactive_complaints_table"])
    if pc_count != 5:
        raise RuntimeError(
            f"[ImprovementRoadmap] proactive_complaints_table must have exactly 5 rows "
            f"(got {pc_count}). Check LLM output."
        )
    ...
    for idx, pc_row in enumerate(raw_data["proactive_complaints_table"]):
        if not pc_row.get("الشكوى المتوقعة", "").strip():
            raise RuntimeError(
                f"[ImprovementRoadmap] Proactive complaints row {idx} has empty 'الشكوى المتوقعة'."
            )
        if not pc_row.get("الأثر", "").strip():
            raise RuntimeError(
                f"[ImprovementRoadmap] Proactive complaints row {idx} has empty 'الأثر'."
            )
        if not pc_row.get("الحل الاستباقي المقترح", "").strip():
            raise RuntimeError(
                f"[ImprovementRoadmap] Proactive complaints row {idx} has empty 'الحل الاستباقي المقترح'."
            )
        if not pc_row.get("مستوى الثقة", "").strip():
            raise RuntimeError(
                f"[ImprovementRoadmap] Proactive complaints row {idx} has empty 'مستوى الثقة'."
            )
