...
def _generate_faq_candidates_by_service(
    ...
    for service_name, cases in sorted(service_groups.items()):
        ...
        # Extract top_level from first case (all cases in inquiries flow should have same top_level)
        service_top_level = (cases[0].top_level or "استفسار").strip() if cases else "استفسار"
...
def _generate_faq_candidates_from_grouped_cases(
    ...
    for (top_level, sub_classification), cases in sorted(groups.items()):
        ...
        prompt = f"""Based on these {len(cases)} customer cases in the category '{sub_classification}',
...
Only include FAQs that are directly supported by the case examples above.
"""
...
def run_stage4(state: PipelineState, api_key: str) -> PipelineState:
    ...
    # TEMPORARILY DISABLED FOR TESTING (matching complaints flow)
    # if not state.faq_candidates and state.all_classified:
