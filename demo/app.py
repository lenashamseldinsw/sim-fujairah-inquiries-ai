...
def inquiries_page(lang):
    ...
    periods = get_period_folders('inquiries')

    if not periods:
        st.error("❌ No inquiry period data available")
        st.markdown('</div>', unsafe_allow_html=True)
        return

    # Initialize selected period with first available period if not already set
    if st.session_state.selected_period_inq is None:
        st.session_state.selected_period_inq = periods[0]

    # Period selection dropdown
    st.markdown(f"""
    <div style="max-width:820px;margin:0 auto 1.5rem;direction:{DIR};">
        <label style="color:{TEXT_MUTED};font-size:0.9rem;font-weight:500;">
            {'اختر الفترة الزمنية' if lang == 'ar' else 'Select Period'}
        </label>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        selected_period = st.selectbox(
            "Period",
            options=periods,
            index=periods.index(st.session_state.selected_period_inq) if st.session_state.selected_period_inq in periods else 0,
            label_visibility="collapsed",
            key="inq_period_select"
        )
        if selected_period != st.session_state.selected_period_inq:
            st.session_state.selected_period_inq = selected_period
    ...
    if st.session_state.processing:
        ...
        """, unsafe_allow_html=True)
        # Simulate processing
        simulate_period_processing(lang)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.session_state.completed:
        ...
        # Display report outputs in tabs for selected period
        st.markdown('<div style="max-width:900px;margin:2rem auto;">', unsafe_allow_html=True)
        display_report_tabs(lang, flow_type='inquiries', period=st.session_state.selected_period_inq)
        st.markdown('</div>', unsafe_allow_html=True)

        # Check if files exist in the selected period
        docx_path, xlsx_path = get_report_files_in_period('inquiries', st.session_state.selected_period_inq)
        if docx_path and docx_path.exists():
            zip_data = create_download_zip(flow_type='inquiries', period=st.session_state.selected_period_inq)
...
def complaints_page(lang):
    ...
    # Get available periods
    periods = get_period_folders('complaints')

    if not periods:
        st.error("❌ No complaint period data available")
        st.markdown('</div>', unsafe_allow_html=True)
        return

    # Initialize selected period with first available period if not already set
    if st.session_state.selected_period_cmp is None:
        st.session_state.selected_period_cmp = periods[0]
    ...
    with col2:
        selected_period = st.selectbox(
            "Period",
            options=periods,
            index=periods.index(st.session_state.selected_period_cmp) if st.session_state.selected_period_cmp in periods else 0,
            label_visibility="collapsed",
            key="cmp_period_select"
        )
        if selected_period != st.session_state.selected_period_cmp:
            st.session_state.selected_period_cmp = selected_period
            st.session_state.processing = False
            st.session_state.completed = False
            st.rerun()
    ...
    if st.session_state.completed:
        ...
        # Display report outputs in tabs for selected period
        st.markdown('<div style="max-width:900px;margin:2rem auto;">', unsafe_allow_html=True)
        display_report_tabs(lang, flow_type='complaints', period=st.session_state.selected_period_cmp)
        st.markdown('</div>', unsafe_allow_html=True)
