...
class DynamicReportDisplay:
    ...
    def _display_section(self, section: Dict[str, Any], all_charts: List[Dict]) -> None:
        ...
        if section.get('subsections'):
            for subsec in section['subsections']:
                ...
                # Show subsection charts FIRST (pie charts in horizontal row, then others)
        ...
        # Pie charts first in horizontal row, then other charts
        if all_charts:
            st.markdown("<br>", unsafe_allow_html=True)
    ...
    def _display_charts_with_pie_priority(self, charts: List[Dict]) -> None:
        """
        Display charts with pie/doughnut charts in horizontal row first.

        Args:
            charts: List of chart data dictionaries
        """
        # Separate pie charts from others
        ...
        # Display pie charts in horizontal row
        if pie_charts:
            num_cols = min(len(pie_charts), 3)  # Max 3 pie charts per row
        ...
        # Display other charts (bar, line, etc.)
        if other_charts:
            if pie_charts:
                st.markdown("<br>", unsafe_allow_html=True)
